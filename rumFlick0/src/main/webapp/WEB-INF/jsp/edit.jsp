...
<body>
	...
	%>
	<h2>회원 정보 수정</h2>
<%-- 	<form name="member" action="edit_ok.jsp?no=<%=no%>" method="post"> --%>
	<form name="member" action="/rum/editok.do" method="post">
		...
		전화번호 : <select name="mTel1">
				...
				<option value="012" <% if(mTel.substring(0,3).equals("012")){%>selected<%}%>>012</option>
		...
		</select>-<input type="text" name="mTel2" value='<%=mTel.substring(4,8)%>'>-<input type="text" name="mTel3" value='<%=mTel.substring(9, 13)%>'>
		<p></p>
		이메일 : <input type="text" name="mEmail1" value="<%=mEmail1%>">@<select name="mEmail2">
